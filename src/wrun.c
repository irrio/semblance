...
wasm_func_addr_t *wrun_store_alloc_funcs(WasmStore *store, WasmModuleInst *winst, VEC(WasmFunc) *funcs) {
...
wasm_table_addr_t *wrun_store_alloc_tables(WasmStore *store, VEC(WasmTable) *tables) {
...
wasm_mem_addr_t *wrun_store_alloc_mems(WasmStore *store, VEC(WasmMemType) *mems) {
...
wasm_global_addr_t *wrun_store_alloc_globals(WasmStore *store, VEC(WasmGlobalType) *globals, VEC(WasmValue) *globalinit) {
...
wasm_elem_addr_t *wrun_store_alloc_elems(WasmStore *store, VEC(WasmElem) *elems, VEC(VEC(WasmRefValue)) *references) {
...
wasm_data_addr_t *wrun_store_alloc_datas(WasmStore *store, VEC(WasmData) *wdatas) {
...
WasmExportInst *wrun_instance_assign_exports(VEC(WasmExport) *exports, VEC(WasmExternVal) *imports, WasmModuleInst *winst) {
    WasmDecomposedImports decomposed;
    wrun_decompose_imports(imports, &decomposed);

    VEC(WasmExportInst) out;
    vec_init(&out);
        ...
        switch (wexp->desc.kind) {
            case WasmExportMem: {
                ...
                if (idx < decomposed.mems.len) {
                    ...
                    inst.val.val.mem = winst->memaddrs[idx - decomposed.mems.len];
                }
                break;
            }
            case WasmExportFunc: {
                ...
                if (idx < decomposed.funcs.len) {
                    ...
                    inst.val.val.func = winst->funcaddrs[idx - decomposed.funcs.len];
                }
                break;
            }
            case WasmExportTable: {
                ...
                if (idx < decomposed.tables.len) {
                    ...
                    inst.val.val.table = winst->tableaddrs[idx - decomposed.tables.len];
                }
                break;
            }
            case WasmExportGlobal: {
                ...
                if (idx < decomposed.globals.len) {
                    ...
                    inst.val.val.global = winst->globaladdrs[idx - decomposed.globals.len];
                }
                break;
            }
        }
        vec_push_back(&out, sizeof(WasmExportInst), &inst);
    }
...
WasmModuleInst *wrun_store_alloc_module(WasmStore *store, WasmModule *wmod, WasmInitParams *params) {
    ...
    winst->funcaddrs = wrun_store_alloc_funcs(store, winst, &wmod->funcs);
    winst->tableaddrs = wrun_store_alloc_tables(store, &wmod->tables);
    winst->memaddrs = wrun_store_alloc_mems(store, &wmod->mems);
    winst->globaladdrs = wrun_store_alloc_globals(store, &wmod->globals, &params->globalinit);
    winst->elemaddrs = wrun_store_alloc_elems(store, &wmod->elems, &params->references);
    winst->dataaddrs = wrun_store_alloc_datas(store, &wmod->datas);
